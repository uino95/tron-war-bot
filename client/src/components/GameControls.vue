                        ...
                        <v-card>
                            ...
                            <v-card-title primary-title class="justify-center">
                                <v-form ref="form" v-model="valid" lazy-validation>
                                    <v-layout row wrap>
                                        <v-flex md4>
                                            <v-autocomplete outline v-model="currentCountry" :items="mapping"
                                                            item-text="name" :loading="isLoading"
                                                            :search-input.sync="search" item-value="numberId"
                                                            hide-no-data hide-selected label="Select Country"
                                                            placeholder="Type in or pick from map"></v-autocomplete>
                                        </v-flex>
                                        <v-flex md4>
                                            <v-text-field :value="calculatePotentialWin" label="Potential win" outline
                                                          disabled></v-text-field>
                                        </v-flex>
                                        <v-flex md4>
                                            <v-text-field :value="turnTimer" label="Next Turn" outline
                                                          disabled></v-text-field>
                                        </v-flex>
                                    </v-layout>
                                    <v-layout row wrap>
                                        <v-flex md4>
                                            <v-text-field :value="balance?(balance + ' TRX'):'no account'"
                                                          label="Your Balance" outline disabled></v-text-field>
                                        </v-flex>
                                        <v-flex md4>
                                            <v-text-field :value="info.jackpot?(info.jackpot + ' TRX'):'loading...'"
                                        ...
                                        <v-flex md4>
                                            <!--<v-select
                                                                                ...
                                                                                outline
                                                                        ></v-select>-->
                                            <v-text-field v-model="currency" label="Currency" outline
                                                          disabled></v-text-field>
                                        </v-flex>
                                    </v-layout>
                                    <v-btn v-if="turnTimer == '00:00'" color="info" @click="battleInProgress">Battle in
                                        progress...
                                    </v-btn>
                                    <v-btn v-else color="success" @click="placeBet">Bet 50 {{currency}}</v-btn>
                                </v-form>
                            </v-card-title>
                            <v-snackbar v-model="snackbar" :color="snackbarColor" :timeout="snackbarTimeout" vertical
                                        bottom>
                                        ...
                                        <v-flex xs3 style="text-align: start" class="subheading">
                                            <v-tooltip bottom>
                                                <template v-slot:activator="{ on }">
                                                    <span v-on="on" v-text="bet.address.substring(0,8)+'...'"
                                                          v-bind:alt="bet.address"></span>
                                                </template>
                                        ...
                                        <v-flex xs2>
                                            <v-avatar size="95%">
                                                <v-lazy-image src-placeholder="/img/flags/placeholder.svg"
                                                              :src="getFlagString(country[0])"
                                                              onerror="this.onerror=null;this.src='/img/flags/placeholder.svg';"
                                                              :alt="country[0]">
                                                </v-lazy-image>
                                            </v-avatar>
                                        </v-flex>
                    ...
                    <v-flex d-flex sm12 md8 grow>
                        <v-card>
                            ...
                            <v-container grid-list-md text-xs-center class="font-weight-regular gameTab">
                                ...
                                <v-divider class="gameTabDivider"></v-divider>
                                <v-container class="gameTabContent">
                                    <v-layout row wrap v-for="conquest in history.slice().reverse()"
                                              :key="conquest.turn">

  ...
  export default {
    ...
    mounted() {
      ...
      db.ref('public/countriesMap').on('child_changed', (snapshot) => {
        ...
        data.cohesion = (data.cohesion * 100).toFixed(2);
        data['controllerCohesion'] = (this.countriesData[data.occupiedBy].cohesion);
        data['color'] = this.colorsDefault[data.occupiedBy];

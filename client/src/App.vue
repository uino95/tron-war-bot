    ...
    >
        ...
        <template>
            <v-layout class="justify-center">
                <olympic-spinner></olympic-spinner>
            </v-layout>
        </template>
      ...
      <v-list dense dark>
        <template v-for="(item, i) in menuItems">
          <v-layout :class="item.class" v-if="item.heading" :key="i" row align-center>
            <v-flex xs6>
              <v-subheader class="text_primary--text" v-if="item.heading">{{ item.heading }}</v-subheader>
            </v-flex>
          </v-layout>
          ...
          <v-list-tile :class="item.class" v-else :key="i" @click="openLink(item.body)" dark>
            ...
            <v-list-tile-content>
              <v-list-tile-title class="text_primary--text">{{ item.text }}</v-list-tile-title>
            </v-list-tile-content>
          </v-list-tile>
        </template>
      </v-list>
    ...
    <v-toolbar color="secondary" dark app fixed clipped-left>
      ...
      <v-toolbar-items>
        ...
        <v-btn
          ...
          icon
          dark
          @click="openLink('https://t.me/joinchat/J8ocIxZoXsD4stn4nxg24A')"
        >
...
import olympicSpinner from "./components/olympicSpinner.vue"
...
export default {
  ...
  components: {
    Loading,
    "olympic-spinner": olympicSpinner
  },
  data: () => ({
    snackbar: true,
    loading: true,
  ...
  filters: {
    WAR: amount => {
      return amount != null
        ? amount.div("1000000000000000000").toFixed(3) + " WAR"
        : 0 + " WAR";
    }
  },
  computed: {
